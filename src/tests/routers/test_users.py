...
async def test_create_user_email_validation_error(test_app_unauthorized):
    with pytest.raises(ValidationError):
        ...
        registered_user = await test_app_unauthorized.put(url="/users", json=user.model_dump())
...
@pytest.mark.asyncio
async def test_read_users(test_app):
    jobs = await test_app.get("/users")
    assert jobs.status_code == status.HTTP_200_OK


@pytest.mark.asyncio
async def test_update_user(test_user, test_app):
    ...
    updated_user = await test_app.put(url="/users", json=updated_test_user.model_dump())
...
@pytest.mark.asyncio
async def test_update_user_to_same_email(test_user: User, test_app):
    updated_test_user = UserUpdateSchema(
        name='new_name',
...
@pytest.mark.asyncio
async def test_update_user_to_blocked_email(sa_session, test_user: User, test_app):
    old_user = UserFactory.build()
    sa_session.add(old_user)
    sa_session.flush()
...
@pytest.mark.asyncio
async def test_delete_user(test_app, test_user: User):
    deleted_user = await test_app.delete(url="/users")
